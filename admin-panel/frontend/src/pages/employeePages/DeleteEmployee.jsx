...
const DeleteEmployee = () => {
  ...
  const handleDelete = () => {
    ...
    axios.delete(`http://localhost:5001/employees/${id}`)
      .then(() => { setDeleting(false); navigate("/employees"); })
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="ml-52 flex-1 p-8">
        <div className="mb-6"><h1 className="text-2xl font-bold text-gray-800">Delete Employee</h1></div>
        {loading ? <Spinner /> : employee ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 max-w-lg p-8">
            <div className="flex flex-col items-center text-center mb-6">
              ...
              <p className="text-gray-500 text-sm">This will permanently delete this employee record.</p>
            </div>
            <div className="bg-red-50 border border-red-100 rounded-lg p-4 mb-6 space-y-2">
              <div className="flex justify-between text-sm"><span className="text-gray-500">Name</span><span className="font-semibold text-gray-800">{employee.name}</span></div>
              <div className="flex justify-between text-sm"><span className="text-gray-500">Employee ID</span><span className="text-gray-800">{employee.employeeId}</span></div>
              <div className="flex justify-between text-sm"><span className="text-gray-500">Role</span><span className="text-gray-800">{employee.role}</span></div>
            </div>
